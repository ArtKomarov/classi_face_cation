...
TrainTestData::TrainTestData(std::string dataset_name, float test_size = 0) :
    ...
    uli train_size = (uli)labels.size();
    //Split on train/test data
    if(test_size != 0) {
        auto img_begin = images.begin();
        train_size = uli((1 - test_size) * labels.size());

        test_images.assign( img_begin + train_size + 1, images.end() );
        test_labels.assign( labels.begin() + train_size + 1, labels.end() );
    ...
    test_ = torch::data::make_data_loader<torch::data::samplers::SequentialSampler>(
                std::move(test_custom_dataset),
                BATCH_SIZE
                );

    // Information for user
    std::cout << "End processing images" << std::endl;
    std::cout << "Train data size: "     << train_size << std::endl;
    std::cout << "Test data size: "      << train_size / (1 - test_size) * test_size << std::endl;
...
CustomDataset::CustomDataset(std::vector<torch::Tensor> &images, std::vector<torch::Tensor> &labels) :
